...
uint32_t limit_duty(uint32_t duty) //限幅函数
{
    if (duty > 4000)
    {
        duty = 4000;
...
uint16_t PWM_A_duty = 0;
uint16_t PWM_B_duty = 0;

float target_speed_A = 0; //目标速度
float target_speed_B = 0; //目标速度    
float last_error_A = 0; //上一次误差
float last_error_B = 0; //上一次误差    
float current_error_A = 0; //当前误差
float current_error_B = 0; //当前误差
...
void DC_Motor_PID(uint8_t motor_id)
{
    ...
    if (motor_id == 1)
    {
        ...
        PWM_A_duty += (uint16_t)(kp * (current_error_A-last_error_A) + ki *(current_error_A));
        last_error_A = current_error_A;
        PWM_A_duty = limit_duty(PWM_A_duty);
        motor_set_duty(motor_id, PWM_A_duty);
    }
    else if (motor_id == 2)
    {
        error = target_speed_B - speed_B;
        current_error_B = error;
        PWM_B_duty += (uint16_t)(kp * (current_error_B-last_error_B) + ki *(current_error_B));
        last_error_B = current_error_B;
        PWM_B_duty = limit_duty(PWM_B_duty);
        motor_set_duty(motor_id, PWM_B_duty);
    }
}
